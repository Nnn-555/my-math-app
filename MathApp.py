...
# 1. PAGE SETUP
st.set_page_config(layout="wide", page_title="Math Mastery AI", page_icon="📐")

# 2. LOAD AI BRAIN - Optimized for Free Tier
if "GEMINI_API_KEY" in st.secrets:
    try:
        # We ONLY set up the connection. No "test" messages to save quota.
        raw_key = st.secrets["GEMINI_API_KEY"].strip()
        client = genai.Client(api_key=raw_key)
        st.sidebar.success("AI Brain Ready ✅")
    except Exception as e:
        st.sidebar.error("Check API Key in Secrets")
else:
    ...
    if user_data["syllabus"]:
        ...
        if uploaded_file:
            with st.spinner("Reading PDF..."):
                ...
                st.success("Syllabus Saved! Go to Quiz.")

# --- MENU 2: QUIZ ---
elif menu == "📝 Quiz":
    st.subheader("Multiple Choice Quiz")
    if not user_data["syllabus"]:
        st.warning("⚠️ Please upload a PDF in the 'File' tab first.")
    else:
        if st.button("Generate New Question"):
            ...
            try:
                # Using 2.0-flash as it is the most modern version for the new library
                res = client.models.generate_content(model="gemini-2.0-flash", contents=prompt)
                st.session_state.current_q = res.text
            except Exception as e:
                st.error("AI is resting. Please wait 1 minute and try again.")
...
# --- MENU 3: COMPREHENSION ---
elif menu == "🧠 Comprehension":
    st.subheader("Handwriting Analysis")
    target_q = "Solve for x: 2x + 10 = 30"
    st.write(f"**Question:** {target_q}")
    canvas_result = st_canvas(fill_color="white", stroke_width=4, stroke_color="black", background_color="#ffffff", height=400, key="comp_canvas")
    ...
    if st.button("Analyze My Work"):
        if canvas_result.image_data is not None:
            ...
            try:
                prompt = f"Question: {target_q}. Check these steps. Are they right?"
                response = client.models.generate_content(model="gemini-2.0-flash", contents=[prompt, white_bg])
                st.info(response.text)
            except Exception as e:
                st.error("AI is resting. Please wait 1 minute.")
